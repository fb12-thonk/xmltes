...
export default function AdsXmlAdmin() {
  ...
  // State Animasi Tombol Copy & Delete
  const [copiedId, setCopiedId] = useState(null);
  const [deletingId, setDeletingId] = useState(null);

  // Kredensial Supabase langsung dari Environment Vercel lu
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

  // Headers wajib buat API Supabase
  const getHeaders = () => ({
    'apikey': supabaseKey,
  ...
  // SIMPAN DATA (POST) TANPA PLUGIN
  const handleCreate = async (e) => {
    e.preventDefault();
    setIsSubmitting(true);
    
    const res = await fetch(`${supabaseUrl}/rest/v1/vast_ads`, {
      method: 'POST',
  ...
  // HAPUS DATA (DELETE) TANPA PLUGIN
